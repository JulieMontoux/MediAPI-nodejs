...
exports.createEmprunt = async (req, res) => {
  ...
  try {
    const emprunt = await empruntRepository.createEmprunt({ id_livre, email, nom, prenom });
...
exports.updateEmprunt = async (req, res) => {
  ...
  try {
    ...
    res.setHeader('ETag', emprunt.etag);
    res.status(200).json({ success: true, message: 'Date de retour mise à jour', data: emprunt });
  } catch (error) {
